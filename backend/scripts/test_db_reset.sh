...
# bcrypt hash below is a one-time `hash_password("test")` value (verify is stable
# for any valid bcrypt hash); updated_at is pinned to the golden's frozen
# 2024-01-01 so the reset stays deterministic. The min_length=8 /api/setup/claim
# guard does not apply here — this mirrors the lifespan pre-seed, which sets the
# 4-char "test" password directly (global-setup login is unchanged).
# ADMIN_HASH is a TRUSTED, code-controlled constant, a one-time
# hash_password("test") value. It is interpolated into the SQL heredoc below,
# which is safe ONLY because the value is fixed here and bcrypt's alphabet
# excludes the single-quote that would break/escape the string. This value must
# NEVER be derived from external/user input; if it ever needs to come from a
# variable input, switch to a parameterized statement (sqlite3 bind / .param)
# instead of string interpolation.
ADMIN_HASH='$2b$12$zAa7bsAIyrbL9bE3I4psEuVDBwCL.mOfxTn/SF58nAh7cONT5hJcy'
