...
async def _delete_lot_allocs_for_sell(
    session: AsyncSession, sell_txn_id: str
) -> None:
    """Delete LotAlloc rows for a sell/spend before recomputing FIFO."""
    await session.execute(
        sql_delete(LotAlloc).where(LotAlloc.sell_txn_id == sell_txn_id)
    )


async def _recompute_fifo_for_later_sells(
    session: AsyncSession,
    account_id: str,
    instrument_id: str,
    after_date: date,
    ...
    whose date >= after_date. Required after a back-dated NEGATIVE
    adjustment because the new adjustment competes for lots ahead of
    existing sells in FIFO order.
    """
    stmt = (
        select(Transaction)
        .where(
    ...
    for sell_txn in result.scalars().all():
        await _delete_lot_allocs_for_sell(session, sell_txn.id)
        await session.flush()
...
async def _current_qty_map(
    ...
    via _recompute_fifo_for_later_sells when negative); it is not an
...
async def save_event(
    ...
    for decision in payload.decisions:
        ...
        if decision.action == "accept":
            ...
            await _recompute_fifo_for_later_sells(
                session,
                payload.account_id,
